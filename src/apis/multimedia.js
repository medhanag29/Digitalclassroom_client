import { BASEML, GENERATE_MCQ_ENDPOINT, GENERATE_LESSON_TEST_ENDPOINT } from "../constants/endpoints";
...
export const generateMCQ = async (text, count = 5) => {
  const response = await fetch(GENERATE_MCQ_ENDPOINT, {
    method: "POST",
  ...
  const data = await response.json();
  return data.generated_mcqs || [];
};

export const generateLessonTest = async (file, source = "hi", language = "en", count = 5) => {
  const formData = new FormData();
  ...
  const response = await fetch(GENERATE_LESSON_TEST_ENDPOINT, {
    method: "POST",
    body: formData,
  });
