...
import { getResponsesFromImage, generateMCQ } from "../apis/multimedia";
// mui
import {
  Container,
  Grid,
...
const Test = () => {
  ...
  const [mcqSourceText, setMcqSourceText] = useState("");
  const [generatedMCQs, setGeneratedMCQs] = useState([]);
  const [isGeneratingMCQs, setIsGeneratingMCQs] = useState(false);
  const [savingGeneratedQuestions, setSavingGeneratedQuestions] = useState(false);
  ...
  const handleGenerateMCQs = async () => {
    if (!mcqSourceText.trim()) {
      alert("Paste lesson text or topic before generating MCQs.");
      return;
    }
    setIsGeneratingMCQs(true);
    try {
      const generated = await generateMCQ(mcqSourceText, 5);
  ...
  const handleSaveGeneratedQuestions = async () => {
    if (!generatedMCQs.length || !test) {
      return;
    }
    setSavingGeneratedQuestions(true);
    try {
      const createdQuestions = [];
      for (const mcq of generatedMCQs) {
        const res = await axios.post(QUESTION_NEW_ENDPOINT, mcq, {
          headers: { Authorization: `Bearer ${token}` },
        });
        if (res.data.data) {
          createdQuestions.push(res.data.data);
        }
      }

      if (createdQuestions.length) {
        const newQuestionIds = createdQuestions.map((item) => item._id);
        const updatedQuestionList = [...(test.questions || []), ...newQuestionIds];

        await axios.patch(
          TEST_EDIT_ENDPOINT,
          {
            query: { _id: testId },
            edits: { questions: updatedQuestionList },
          },
          { headers: { Authorization: `Bearer ${token}` } }
        );
        ...
        setGeneratedMCQs([]);
        setMcqSourceText("");
        alert(`Saved ${createdQuestions.length} generated questions to this test.`);
      }
    } catch (err) {
      console.error(err);
      alert("Failed to save generated questions. Please try again.");
    } finally {
      setSavingGeneratedQuestions(false);
    }
  };
                  ...
                  <Grid container spacing={2}>
                    ...
                    <Grid item xs={12}>
                      <TextField
                        ...
                        rows={4}
                        value={mcqSourceText}
                        onChange={(e) => setMcqSourceText(e.target.value)}
                        label="Paste lesson text/topic for auto MCQ generation"
                        variant="outlined"
                      />
                    </Grid>
                    <Grid item xs={12}>
                  ...
                  <Box mt={3} p={2} bgcolor="#f9f9f9" borderRadius={2}>
                    <Stack direction="row" justifyContent="space-between" mb={2}>
                      ...
                      <LoadingButton
                        ...
                        loading={savingGeneratedQuestions}
                      >
                      ...
                      <Box key={index} mb={2} p={2} border="1px solid #ddd" borderRadius={2}>
                        ...
                        <List dense>
                          {mcq.options.map((option) => (
                            <ListItemText
                              key={option.key}
                              primary={`${option.key}. ${option.value}`}
                            />
                          ))}
                        </List>
